...
ans <- vector("character", ncol(flights))
for (i in seq(ans)) {
  ans[[i]] <- typeof(nycflights13::flights[[i]])
}
ans

length(unique(iris[[1]]))
ans <- vector("double", ncol(iris))
for (i in seq_along(ans)) {
  ans[[i]] <- length(unique(iris[[i]]))
}
ans

norm_means <- c(-10, 0, 10, 100)
ans <- vector("list", length(norm_means))
for (i in seq_along(ans)){
  ans[[i]] <- rnorm(10, mean = norm_means[[i]])
}
ans
